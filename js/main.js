...
function Form (model, options) {
	...
	function validateForm (changes) {
		...
		function toggleStage ($group, $label, changed) {
			$group.setAttribute('class', model.attributes[changed.name].isValid ? 'group valid' : 'group invalid');
	...
	options.el.addEventListener('submit', function (event) {
		...
		if(model.formValid()) {
			options.el.innerHTML = "<h2>Form submitted</h2><p><strong>Thanks for registering " + model.attributes.firstName.value + "!</strong></p> <p>We'll be sending you an email to <strong>" + model.attributes.email.value + "</strong> regarding activating your account.</p>";
		} else {
			for(var key in model.attributes) {
